...
{
    static_assert((... && is_container_v<Args>), "sum only over containers");
    //return (... + std::accumulate(std::begin(args), std::end(args), 0)); // see zero arguments subcase
    return (0 + ... + std::accumulate(std::begin(args), std::end(args), 0));
}
...
struct summation
{
    // template of member function/call-operator:
